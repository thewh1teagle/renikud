...
class SwiGLU(nn.Module):
    """SwiGLU FFN with packed gate/up projection (same layout as xformers.ops.SwiGLU, bias=False)."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.w12 is not None:
            w = self.w12.weight.view(2, self.hidden_features, self.in_features)
            w1, w2 = w[0], w[1]
            if self.w12.bias is not None:
                b = self.w12.bias.view(2, self.hidden_features)
                x1 = F.linear(x, w1, b[0])
                x2 = F.linear(x, w2, b[1])
            else:
                x1 = F.linear(x, w1, None)
                x2 = F.linear(x, w2, None)
...
class NeoBERT(NeoBERTPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Reshape for broadcast: (Batch, Length) -> (Batch, 1, 1, Length)
        if attention_mask is not None:
        ...
        # RoPE: 2D (seq, dim//2) like LLaMA when positions are consecutive; 3D when indexing by position_ids
